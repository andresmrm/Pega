...
char PASTA[100]="modelos/";
char PASTA_T[100]="texturas/";
...
bool ModelType::LoadObj(char *filename2, float Scale)
{
	char filename[100] = {};
	strcat(filename,PASTA);
...
void ModelType::GenDispObj()
{
   for (int i = 0; i < numObjects; i++)
   {
      ...
      {
         if (!currPtr->NormType)
            glNormal3f(currPtr->faceNorm.x
            		, currPtr->faceNorm.y
            		, currPtr->faceNorm.z);
         else
            glNormal3f(tDispObj->objNorms[currPtr->n1].x
            		, tDispObj->objNorms[currPtr->n1].y
            		, tDispObj->objNorms[currPtr->n1].z);
         if (currPtr->t1 >= 0)
            glTexCoord3f(tDispObj->objTexts[currPtr->t1].u
            		, tDispObj->objTexts[currPtr->t1].v
            		, tDispObj->objTexts[currPtr->t1].w);
         glVertex3f(tDispObj->objVerts[currPtr->v1].x
        		 , tDispObj->objVerts[currPtr->v1].y
        		 , tDispObj->objVerts[currPtr->v1].z);

         if (currPtr->NormType)
            glNormal3f(tDispObj->objNorms[currPtr->n2].x
            		, tDispObj->objNorms[currPtr->n2].y
            		, tDispObj->objNorms[currPtr->n2].z);
         if (currPtr->t2 >= 0)
            glTexCoord3f(tDispObj->objTexts[currPtr->t2].u
            		, tDispObj->objTexts[currPtr->t2].v
            		, tDispObj->objTexts[currPtr->t2].w);
         glVertex3f(tDispObj->objVerts[currPtr->v2].x
        		 , tDispObj->objVerts[currPtr->v2].y
        		 , tDispObj->objVerts[currPtr->v2].z);

         if (currPtr->NormType)
            glNormal3f(tDispObj->objNorms[currPtr->n3].x
            		, tDispObj->objNorms[currPtr->n3].y
            		, tDispObj->objNorms[currPtr->n3].z);
         if (currPtr->t3 >= 0)
            glTexCoord3f(tDispObj->objTexts[currPtr->t3].u
            		, tDispObj->objTexts[currPtr->t3].v
            		, tDispObj->objTexts[currPtr->t3].w);
         glVertex3f(tDispObj->objVerts[currPtr->v3].x
        		 , tDispObj->objVerts[currPtr->v3].y
        		 , tDispObj->objVerts[currPtr->v3].z);

         if (currPtr->NormType)
            glNormal3f(tDispObj->objNorms[currPtr->n4].x
            		, tDispObj->objNorms[currPtr->n4].y
            		, tDispObj->objNorms[currPtr->n4].z);
         if (currPtr->t4 >= 0)
            glTexCoord3f(tDispObj->objTexts[currPtr->t4].u
            		, tDispObj->objTexts[currPtr->t4].v
            		, tDispObj->objTexts[currPtr->t4].w);
         glVertex3f(tDispObj->objVerts[currPtr->v4].x
        		 , tDispObj->objVerts[currPtr->v4].y
        		 , tDispObj->objVerts[currPtr->v4].z);
